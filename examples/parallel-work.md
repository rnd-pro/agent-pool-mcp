...
Use `.agent-portal/delegation/` in the project root:

```
.agent-portal/delegation/
...
1. **Check findings files FIRST** — agent may have written partial results before timeout
   ```bash
   ls .agent-portal/delegation/findings-*.md   # ALWAYS check regardless of timeout
   ```
...
```
0. mkdir -p .agent-portal/delegation/         ← ensure dir exists BEFORE delegating
1. delegate_task_readonly(research, timeout=300)
2. WAIT with get_task_result() — poll every 30-60s
3. On timeout → ls .agent-portal/delegation/  ← check for partial results
4. READ findings from .agent-portal/delegation/findings-*.md
